...
There were 1248 movies made in this year.
...
```{r}
movies %>%
  filter(length > 120 & rating > 8 & Comedy == 1) %>%
  drop_na(mpaa)
```

47 Movies fit this description.
...
```{r}
movies %>%
  select(title, rating, budget, votes) %>%
  filter(str_detect(title, "Harry Potter"))
```

The Chamber of Secrets, Prisoner of Azkaban, and Sorcerers Stone are in the data set. Prisoner of Azkaban had the highest rating and Sorcerers Stone has the most votes.
...
```{r}
movies %>%
  filter(votes >= 100 & (Comedy == 1 | Animation == 1)) %>%
  select(title, budget, votes) %>%
  arrange(desc(budget))
```

The film "Wild Wild West" had the highest budget for comedies or animations with more than 100 votes.
...
```{r}
movies %>%
  filter(str_detect(title, "Star Wars") | str_detect(title, "Lord of the Rings")) %>%
  select(title,year,rating) %>%
  arrange(desc(rating))
```

Yes, Star Wars: Episode VI is listed, and is my favorite film.


We could ask questions using the "or" function and arrange by rating. From this point we could demonstrate that a different series, such as the Lord of the Rings, has an overall higher rating than the star wars movies that are listed.
